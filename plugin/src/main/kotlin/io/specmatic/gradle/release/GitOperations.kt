...
enum class CommitType(private val messageSubstring: String) {
    PRE_RELEASE("pre-release"), POST_RELEASE("post-release");

    fun commitMessage(version: String): String {
        return "chore(release): $messageSubstring bump version $version"
    }
}

fun File.execGit(logger: Logger, vararg args: String, quiet: Boolean = false): ProcessResult {
    val processExecutor =
        ProcessExecutor().command("git", *args).directory(this).readOutput(true).timeout(30, TimeUnit.SECONDS)
            .destroyOnExit()

    return processExecutor.execute().also {
        if (it.exitValue == 0) {
            if (!quiet) {
                logger.warn("Command '${processExecutor.command}' executed successfully in directory $this. Output was: ${it.outputUTF8()}")
            }
        } else {
            throw RuntimeException("Command '${processExecutor.command}' failed in directory $this with exit code ${it.exitValue}. Output was: ${it.outputUTF8()}")
        }
    }
}
...
class GitOperations(
    private val rootDir: File, private val projectProperties: Map<String?, Any?>, private val logger: Logger
) {
    ...
    private fun localGitTags(): List<String> =
        rootDir.execGit(logger, "tag", quiet = true).outputUTF8().lines().map { it.trim() }

    private fun extractRemoteTags(): List<String> {
        return rootDir.execGit(logger, "ls-remote", "--tags", quiet = true).outputUTF8().lines().mapNotNull { line ->
    ...
    fun assertMainBranch() {
        ...
        val currentBranch =
            rootDir.execGit(logger, "branch", "--no-color", "--show-current").outputUTF8().lines().first().trim()
        logger.warn("Current branch: $currentBranch")

        if (currentBranch != "main") {
            throw GradleException("You are not on the main branch. Please switch to the main branch to create a release. Run with `-PskipBranchCheck=true` to disable.")
        }
    }
...
//        }
    ...
    fun assertNoIncomingOrOutgoingChanges() {
        ...
        val status = rootDir.execGit(logger, "status", "-b", "--porcelain").outputUTF8().lines().first()
        if (status.contains("ahead")) {
            logger.warn("Repo is ahead of remote. Status: $status")
            throw GradleException("Repo is ahead of remote. Please push your changes before creating a release. Run with `-PskipIncomingOutgoingCheck=true` to disable.")
        } else if (status.contains("behind")) {
            logger.warn("Repo is behind remote. Status: $status")
            throw GradleException("Repo is behind remote. Please pull the latest changes before creating a release. Run with `-PskipIncomingOutgoingCheck=true` to disable.")
        } else {
    ...
    fun gitSha(): String {
        return rootDir.execGit(logger, "rev-parse", "HEAD", quiet = true).outputUTF8().lines().first().trim()
    }
}
